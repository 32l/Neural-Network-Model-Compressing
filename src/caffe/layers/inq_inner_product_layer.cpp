...
void TwoPowerInnerProductLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top)
{
  ...
  TwoPowerInnerProductParameter tpinner_param = this->layer_param_.tpinner_product_param();
  // Initialize the portion array
  this->num_portions_ = tpinner_param.portion().size();
  CHECK_GT(this->num_portions_, 0) << "Number of portions must be greater than 0.";
...
template <typename Dtype>
void TwoPowerInnerProductLayer<Dtype>::Reshape(const vector<Blob<Dtype> *> &bottom,
...
template <typename Dtype>
void TwoPowerInnerProductLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top)
{
  /********** for neural network model compression **********/
  if (this->phase_ == TRAIN)
  {
    if (this->iter_ == 0)
    {
      ...
      {
        ComputeQuantumRange(this->blobs_[0].get(), this->blobs_[2].get(), this->portions_,
                            weight_quantum_values_, num_weight_quantum_values_,
                            max_weight_quantum_exp_, min_weight_quantum_exp_);
        std::cout << "Shaping the weights..." << std::endl;
        ShapeIntoTwoPower(this->blobs_[0].get(), this->blobs_[2].get(),
                          this->portions_,
                          max_weight_quantum_exp_, min_weight_quantum_exp_);

        ComputeQuantumRange(this->blobs_[1].get(), this->blobs_[3].get(), this->portions_,
                            bias_quantum_values_, num_bias_quantum_values_,
                            max_bias_quantum_exp_, min_bias_quantum_exp_);
      ...
      else if (this->blobs_.size() == 2 && (!this->bias_term_))
      {
        ...
        ComputeQuantumRange(this->blobs_[0].get(), this->blobs_[1].get(), this->portions_,
                            weight_quantum_values_, num_weight_quantum_values_,
                            max_weight_quantum_exp_, min_weight_quantum_exp_);
...
template <typename Dtype>
void TwoPowerInnerProductLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype> *> &top,
...
template <typename Dtype>
void TwoPowerInnerProductLayer<Dtype>::ComputeQuantumRange(const Blob<Dtype> *blob, const Blob<Dtype> *blob_mask, const vector<float> portions, vector<Dtype> &quantum_values, const int &num_quantum_values, int &max_quantum_exp_, int &min_quantum_exp_)
{
  ...
  if (portions[0] == 0)
  {
    CHECK_EQ(updated, 0) << updated << " updated values while there should be none!";
  ...
  else
  {
    ...
    CHECK_LE(max_tobe_quantized_exp_, max_quantum_exp_) << "New quantum exp is greater than the one already got!";
  }
...
template <typename Dtype>
void TwoPowerInnerProductLayer<Dtype>::ShapeIntoTwoPower(Blob<Dtype> *input_blob, Blob<Dtype> *mask_blob, const vector<float> &portions, const int &max_quantum_exp_, const int &min_quantum_exp_)
{
  // Get the portions
  ...
  if (update > 0)
  {
    ...
    CHECK_EQ(k, left) << "Num of weights/bias that are not in 2 power form does NOT match the portion!";
    ...
    for (int i = 0; i < count; ++i)
    {
      if (mask[i] == 1)
      {
        ...
        {
          // exp_ won't be larger than max_quantum_exp_, already checked in the ComputeQuantumRange()
